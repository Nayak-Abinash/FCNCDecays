...
//parameters:
double pi=M_PI;
double mB=5.27958,mK=0.497614,mu=4.18,mc=1.27,mb=4.18;
double C1=-0.257,C2=1.009,C3=-0.005,C4=-0.078,C5=0.000,C6=0.001,C7effRe=-0.304,C10effRe=-4.103,
...
//SMsub:
double C7effIm=0.0,C7RHRe=0.0,C7RHIm=0.0,C9RHRe=0.0,C9RHIm=0.0,C10effIm=0.0,C10RHRe=0.0,C10RHIm=0.0,CPRe=0.0,CPIm=0.0,
    CPRHRe=0.0,CPRHIm=0.0,CSRe=0.0,CSIm=0.0,CSRHRe=0.0,CSRHIm=0.0,CTRe=0.0,CTIm=0.0,CT5Re=0.0,CT5Im=0.0;
...
//amplitudes:(see,arXiv:1205.5811)
...
double FVRe(double qsq, double ml){
    return (C9effRe(qsq) + C9RHRe)*fp(qsq) + 2.0*mb/(mB+mK)*(C7effRe + C7RHRe + 4.0*ml/mb*CTRe)*ft(qsq); }
double FVIm(double qsq, double ml){
    return (C9effIm(qsq) + C9RHIm)*fp(qsq) + 2.0*mb/(mB+mK)*(C7effIm + C7RHIm + 4.0*ml/mb*CTIm)*ft(qsq); }

double FARe(double qsq, double ml){
    return (C10effRe + C10RHRe)*fp(qsq); }
double FAIm(double qsq, double ml){
    return (C10effIm + C10RHIm)*fp(qsq); }

double FSRe(double qsq, double ml){
    return (pow(mB,2.0)-pow(mK,2.0))/(2.0*mb)*(CSRe + CSRHRe)*fz(qsq); }
double FSIm(double qsq, double ml){
    return (pow(mB,2.0)-pow(mK,2.0))/(2.0*mb)*(CSIm + CSRHIm)*fz(qsq); }

double FPRe(double qsq, double ml){
    return (pow(mB,2.0)-pow(mK,2.0))/(2.0*mb)*(CPRe + CPRHRe)*fz(qsq) - ml*(C10effRe+C10RHRe)*(fp(qsq)-(pow(mB,2.0)-pow(mK,2.0))/qsq*(fz(qsq)-fp(qsq))); }
double FPIm(double qsq, double ml){
    return (pow(mB,2.0)-pow(mK,2.0))/(2.0*mb)*(CPIm + CPRHIm)*fz(qsq) - ml*(C10effIm+C10RHIm)*(fp(qsq)-(pow(mB,2.0)-pow(mK,2.0))/qsq*(fz(qsq)-fp(qsq))); }


double FTRe(double qsq, double ml){
    return 2.0*sqrt(lambda(qsq))*betal(qsq,ml)/(mB+mK)*CTRe*ft(qsq); }
double FTIm(double qsq, double ml){
    return 2.0*sqrt(lambda(qsq))*betal(qsq,ml)/(mB+mK)*CTIm*ft(qsq); }

double FT5Re(double qsq, double ml){
    return 2.0*sqrt(lambda(qsq))*betal(qsq,ml)/(mB+mK)*CT5Re*ft(qsq); }
double FT5Im(double qsq, double ml){
    return 2.0*sqrt(lambda(qsq))*betal(qsq,ml)/(mB+mK)*CT5Im*ft(qsq); }
...
double clNP(double qsq, double ml){
    return (qsq*(pow(FT5Im(qsq,ml),2.0) + pow(FT5Re(qsq,ml),2.0) +
            ...
            (pow(betal(qsq,ml),2.0)*(pow(FAIm(qsq,ml),2.0) + pow(FARe(qsq,ml),2.0) + pow(FVIm(qsq,ml),2.0) +
                                     pow(FVRe(qsq,ml),2.0))*lambda(qsq))/4.0)*nf(qsq,ml); }
